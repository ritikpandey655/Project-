  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 animate-fade-in pb-20">
      ...
      <div className="text-center mb-6">
        <h2 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-2">Daily Briefing</h2>
        <p className="text-slate-500 dark:text-slate-400">Curated Current Affairs for Exam Prep</p>
      </div>
      ...
      {isRefreshing ? (
        <div className="flex flex-col items-center justify-center py-12 animate-fade-in">
           ...
           <h3 className="text-lg font-bold text-slate-800 dark:text-white">Curating Events...</h3>
           <p className="text-slate-500 text-sm">Checking {selectedMonth} {selectedYear} archives</p>
        </div>
      ) : (
        <div className="space-y-6">
          {news.length === 0 ? (
             <div className="text-center py-12 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                ...
                <p className="text-slate-500 dark:text-slate-400">No news found for this period. Try changing filters.</p>
             </div>
          ) : (
            news.map((item, index) => {
